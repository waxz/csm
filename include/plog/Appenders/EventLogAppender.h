...
namespace plog {
    ...
    public:
        ...
        virtual void write(const Record &record) {
            ...
            ReportEventW(m_eventSource, logSeverityToType(record.getSeverity()),
                         static_cast<WORD>(record.getSeverity()), 0, NULL, 1, 0, logMessagePtr, NULL);
        }
    ...
    public:
        static bool add(const wchar_t *sourceName, const wchar_t *logName = L"Application") {
            ...
            HKEY sourceKey;
            if (0 != RegCreateKeyExW(hkey::kLocalMachine, sourceKeyName.c_str(), 0, NULL, 0, regSam::kSetValue, NULL,
                                     &sourceKey, NULL)) {
                return false;
            }
            ...
            RegSetValueExW(sourceKey, L"EventMessageFile", 0, regType::kExpandSz, kEventMessageFile,
                           static_cast<DWORD>(::wcslen(kEventMessageFile) * sizeof(wchar_t)));
    ...
    private:
        static void getKeyNames(const wchar_t *sourceName, const wchar_t *logName, std::wstring &sourceKeyName,
                                std::wstring &logKeyName) {
